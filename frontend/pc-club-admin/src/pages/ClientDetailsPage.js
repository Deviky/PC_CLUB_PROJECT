  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [clientRes, orderRes, paymentRes] = await Promise.all([
        fetch(`http://localhost:8966/client/get/${id}`, { headers }),
        fetch(`http://localhost:8966/order/get-by-client/${id}`, { headers }),
        fetch(`http://localhost:8966/payment/operations/${id}`, { headers }),
      ]);
